...
<div x-data="catalogFilter('{{ route('catalog.compare') }}')" class="bg-[#0f0f11] min-h-screen relative pb-24">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex flex-col md:flex-row gap-8">
        ...
        {{-- ===== SIDEBAR FILTERS ===== --}}
        <aside class="w-full md:w-1/4 shrink-0 z-20">
            <div class="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 sticky top-28 shadow-xl shadow-black/80">
                <h2 class="text-sm font-bold text-white mb-6 uppercase tracking-wider border-b border-white/10 pb-4 flex items-center gap-2">
                    <svg class="w-4 h-4 text-[#dc2626]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path></svg>
                    Search & Filters
                </h2>

                {{-- Added id="filterForm" to link with the external shape buttons --}}
                <form id="filterForm" x-ref="filterForm" @submit.prevent="submitForm()" action="{{ route('catalog.index') }}" method="GET" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="filter[model_name]" value="{{ request()->input('filter.model_name') }}"
                            ...
                            class="w-full bg-black/40 border border-white/10 text-white placeholder-zinc-600 rounded-lg focus:ring-[#dc2626] focus:border-[#dc2626] px-4 py-2 outline-none transition backdrop-blur-sm">
                    </div>

                    {{-- Dynamic Brand Dropdown (With native option styling fixes) --}}
                    <div>
                        <label class="block text-sm font-medium text-zinc-400 mb-2">Make / Brand</label>
                        <select name="filter[brand_id]" @change="submitForm()" class="w-full bg-black/40 border border-white/10 text-white rounded-lg focus:ring-[#dc2626] focus:border-[#dc2626] px-4 py-2 cursor-pointer transition outline-none backdrop-blur-sm [&>option]:bg-[#0f0f11] [&>option]:text-white">
                    ...
                    <div>
                        ...
                        <select name="filter[year]" @change="submitForm()" class="w-full bg-black/40 border border-white/10 text-white rounded-lg focus:ring-[#dc2626] focus:border-[#dc2626] px-4 py-2 cursor-pointer transition outline-none backdrop-blur-sm [&>option]:bg-[#0f0f11] [&>option]:text-white">
                    ...
                    <div class="pt-2 border-t border-white/10">
                        ...
                        <select name="sort" @change="submitForm()" class="w-full bg-black/40 border border-white/10 text-white rounded-lg focus:ring-[#dc2626] focus:border-[#dc2626] px-4 py-2 cursor-pointer transition outline-none backdrop-blur-sm [&>option]:bg-[#0f0f11] [&>option]:text-white">
                    ...
                    @if(request()->has('filter') || request()->has('sort'))
                        <a href="{{ route('catalog.index') }}" class="block text-center mt-4 text-sm text-[#dc2626] hover:text-white transition font-semibold">
                            Clear All Filters
                        </a>
                    @endif
                </form>
        ...
        <main class="w-full md:w-3/4 flex flex-col gap-6">
            ...
            <div class="p-4 bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl overflow-x-auto hide-scrollbar z-10 shadow-lg shadow-black/40">
                
                @php
                    // SVG Icon Library mapped directly to CarTypeSeeder entries
                    $carIcons = [
                        'Sedan' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7h8l3 4v4a2 2 0 01-2 2H7a2 2 0 01-2-2v-4l3-4z M5 15h14 M7 15v2 M17 15v2" />',
                        'SUV' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M5 10h14l-1.5-4H6.5L5 10z M4 10v6a1 1 0 001 1h14a1 1 0 001-1v-6 M7 17v1.5 M17 17v1.5" />',
                        'Pickup' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M14 9h6v6h-6V9z M4 11h10v4H4v-4z M4 11l2-4h5l2 4 M6 15v2 M17 15v2" />',
                        'Coupe' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 8h8l3 3v3a2 2 0 01-2 2H8a2 2 0 01-2-2v-3l1-3z M5 14h14" />',
                        'Hatchback' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M5 9h9l3 3v4H5V9z M5 9l2-3h5l2 3 M7 16v1.5 M15 16v1.5" />',
                        // Generic fallback icon for Wagon, Van/MPV, Convertible
                        'default' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h8a1 1 0 001-1z M13 8h6a1 1 0 011 1v7a1 1 0 01-1 1h-6" />'
                    ];
                @endphp
                ...
                <div class="flex gap-4 min-w-max">
                    ...
                    <label class="cursor-pointer group">
                        <input type="radio" name="filter[car_type_id]" value="" class="hidden" form="filterForm" @change="submitForm()" {{ !request('filter.car_type_id') ? 'checked' : '' }}>
                        <div class="px-6 py-4 rounded-xl border transition-all duration-300 flex flex-col items-center justify-center min-w-[110px] gap-2
                            ...
                            <svg class="w-8 h-8 opacity-80 group-hover:opacity-100 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @foreach($carTypes as $type)
                        <label class="cursor-pointer group">
                            <input type="radio" name="filter[car_type_id]" value="{{ $type->id }}" class="hidden" form="filterForm" @change="submitForm()" {{ request('filter.car_type_id') == $type->id ? 'checked' : '' }}>
                            <div class="px-6 py-4 rounded-xl border transition-all duration-300 flex flex-col items-center justify-center min-w-[110px] gap-2
            ...
            <div id="catalog-main-content">
                <div class="flex justify-between items-center mb-6 pb-4 border-b border-white/10">
                    ...
                    <div x-show="isSearching" style="display: none;" class="text-[#dc2626] text-sm animate-pulse flex items-center gap-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    @forelse($cars as $car)
                        <div class="relative group bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden hover:border-[#dc2626]/50 hover:shadow-[0_0_20px_rgba(220,38,38,0.15)] transition duration-300 shadow-xl flex flex-col">
                            
                            {{-- Compare Checkbox Overlay --}}
                            <label class="absolute top-4 right-4 z-20 cursor-pointer bg-[#0f0f11]/80 p-2 rounded-lg backdrop-blur-sm border transition duration-300 shadow-lg"
                                   :class="compareList.includes({{ $car->id }}) ? 'border-[#dc2626] bg-[#dc2626]/10' : 'border-white/10 hover:border-white/30'">
                                <input type="checkbox" :value="{{ $car->id }}" @change="toggleCompare({{ $car->id }})" :checked="compareList.includes({{ $car->id }})" class="hidden">
                                <span class="text-xs font-bold uppercase tracking-wider flex items-center gap-2" :class="compareList.includes({{ $car->id }}) ? 'text-[#dc2626]' : 'text-zinc-400'">
                                    <svg x-show="compareList.includes({{ $car->id }})" class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path></svg>
                            ...
                            <div class="p-5 flex-grow flex flex-col justify-between">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-white mb-4 tracking-tight">₱{{ number_format($car->price, 2) }}</p>
                                </div>
