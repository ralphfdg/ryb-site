            ...
            <div class="hidden sm:flex items-center space-x-8">
                ...
                @auth
                    ...
                    <div x-data="{ open: false }" class="relative inline-block text-left ml-4">
                        ...
                        <button @click="open = !open" @click.away="open = false" type="button" class="flex items-center gap-2 px-4 py-2 bg-ryb-dark text-ryb-light font-medium rounded-lg hover:text-white hover:bg-ryb-muted transition border border-ryb-muted cursor-pointer shadow-sm">
                            <span>{{ auth()->user()->name }}</span>
                            {{-- Chevron Icon --}}
                            <svg class="w-4 h-4 text-ryb-light transition-transform duration-200" :class="{'rotate-180': open}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div x-show="open" 
                            ...
                            <div class="py-1">
                                @if(auth()->user()->hasRole('Admin'))
                                    {{-- Admin Controls --}}
                                    <a href="{{ route('admin.dashboard') }}" class="block px-4 py-3 text-sm text-ryb-light hover:bg-ryb-muted hover:text-white transition">
                                        Admin Portal
                                    </a>
                                @else
                                    {{-- Customer Controls --}}
                                    <a href="{{ route('dashboard.appointments.index') }}" class="block px-4 py-3 text-sm text-ryb-light hover:bg-ryb-muted hover:text-white transition">
                                        Appointments & Inquiries
                                    </a>
                                    <a href="{{ route('dashboard.profile') }}" class="block px-4 py-3 text-sm text-ryb-light hover:bg-ryb-muted hover:text-white transition">
                                        Profile Settings
                                    </a>
                                @endif
                                ...
                                <form method="POST" action="{{ route('logout') }}" class="block">
                                    @csrf
                                    <button type="submit" class="w-full text-left px-4 py-3 text-sm text-ryb-red font-medium hover:bg-ryb-muted transition cursor-pointer">
                                        Log out
                                    </button>
                                </form>
                            </div>
